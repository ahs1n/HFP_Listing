...
import static edu.aku.hassannaqvi.hfplisting.core.MainApp.TAJIKISTAN;
...
import static edu.aku.hassannaqvi.hfplisting.database.CreateTable.DATABASE_COPY;
import static edu.aku.hassannaqvi.hfplisting.database.CreateTable.DATABASE_NAME;
...
import edu.aku.hassannaqvi.hfplisting.core.AppInfo;
import edu.aku.hassannaqvi.hfplisting.core.MainApp;
import edu.aku.hassannaqvi.hfplisting.database.DatabaseHelper;
import edu.aku.hassannaqvi.hfplisting.databinding.ActivityLoginBinding;
...
//                        OutputStream output = new FileOutputStream(outFileName);
    ...
    public void onSyncDataClick(View view) {
        ...
        if (networkInfo != null && networkInfo.isConnected()) {
            startActivity(new Intent(this, edu.aku.hassannaqvi.hfplisting.ui.SyncActivity.class).putExtra("login", true));
        } else {
            ...
            try {
                ...
                if (db.doLogin(username, password)) {
                    MainApp.user.setUserName(username);
                       MainApp.admin = username.contains("user0003") || username.contains("test1234");
